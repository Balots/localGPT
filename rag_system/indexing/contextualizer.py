...
class ContextualEnricher:
    ...
    def enrich_chunks(self, chunks: List[Dict[str, Any]], window_size: int = 1) -> List[Dict[str, Any]]:
        ...
        excel_count = sum(1 for ch in chunks if re.search(r'\.xlsx?$', ch.get('metadata', {}).get('source', ''), re.IGNORECASE))
        enrich_count = len(chunks) - excel_count
        logger.info(f"Enriching {len(chunks)} chunks total: {enrich_count} will be enriched, {excel_count} Excel chunks will be skipped (window_size={window_size})")
        ...
        def process_chunk_batch(chunk_indices):
            ...
            for i in chunk_indices:
                ...
                try:
                    ...
                    if re.search(r'\.xlsx?$', source, re.IGNORECASE):
